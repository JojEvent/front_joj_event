...
export default function EvenementsPage() {
  ...
  const dateOptions = [
    "Toutes les dates",
    ...new Set(
      (events || []).map((event) => event.date_debut).filter(Boolean)
    ),
  ];
        ...
        {error && !isLoading && (
          <div className="self-stretch pb-24 flex flex-col justify-center items-center gap-4">
            <p className="text-red-600 text-base font-bold font-['Olympic_Sans_Bold']">
              Impossible de charger les événements. Vérifiez la connexion à l'API.
            </p>
            <button
              type="button"
              onClick={() => refetch()}
              className="px-6 py-2 bg-green-600 rounded-lg text-white text-sm font-bold font-['Olympic_Sans_Bold'] hover:bg-green-700 transition-colors"
            >
              Réessayer
            </button>
          </div>
